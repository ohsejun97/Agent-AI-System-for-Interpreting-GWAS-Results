"""
Jansen et al. Insomnia GWAS — Step 1~2
입력: dataset/Insomnia_sumstats_Jansenetal.txt.gz
출력: insomnia/results/snp_results.csv, gene_mapping_results.csv
"""
...
print("=" * 55)
print("[Step 1] 유의 SNP 추출 (p < 5×10⁻⁸)  — Insomnia")
print("=" * 55)

df = pd.read_csv(
    DATASET / 'Insomnia_sumstats_Jansenetal.txt.gz',
...
print("\n" + "=" * 55)
print("[Step 2] Ensembl REST API 유전자 매핑 (±500kb, GRCh37)")
print("=" * 55)
...
result_df.to_csv(RESULTS / 'gene_mapping_results.csv', index=False)

print("\n" + "=" * 55)
print("최종 결과 테이블")
print("=" * 55)
print(result_df[['SNP_ID', 'CHR', 'BP', 'P', 'genes']].to_string(index=False))
print(f"\n✓ results/gene_mapping_results.csv 저장 완료 ({len(result_df)}개 SNP)")
